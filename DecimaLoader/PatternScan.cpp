...
Pattern Pattern::from_string(const std::string& pattern) {
    ...
    while (ss.good()) {
        ...
        if (byte == "?" || byte == "??") {
            ...
            bytes.push_back({ .Value = (uint8_t)std::stoul(byte, nullptr, 16) });
        }
    }
...
std::vector<uintptr_t> PatternScanner::scan(const Pattern& pattern) {
    ...
    auto addr = (uint8_t*)module;
    const auto end_addr = addr + module_info.SizeOfImage;
    const auto& bytes = pattern.get_bytes();

    const auto predicate = [](uint8_t byte, Pattern::Byte pattern_byte) {
    ...
    while (addr < end_addr) {
        ...
        const auto begin = (uint8_t*)mbi.BaseAddress;
